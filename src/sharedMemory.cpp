...
sharedMemory::sharedMemory(key_t key, int size, int flag) : m_key(key), m_size(size), m_flag(flag), m_shmid(0), m_ptr(
        nullptr) {
}
...
Sem::Sem(int sem1Value, char *name, int flag, mode_t mode) : m_value(sem1Value), m_name(name), m_flag(flag),
                                                             m_mode(mode) {
